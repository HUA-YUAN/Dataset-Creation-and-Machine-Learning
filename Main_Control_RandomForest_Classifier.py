...
if __name__ == '__main__':
   ...
   if feature == "A":
       ...
       dataset_accleration = dataset.iloc[:,5]
       
   if feature == "B":
       data = dataset.iloc[:,0:4]
       dataset_accleration = dataset.iloc[:,4]
       
   if feature == "C":
       data = dataset.iloc[:,0:7]
       dataset_accleration = dataset.iloc[:,7]
   
   dataset_accleration = dataset_accleration.values
   dataset_accleration = dataset_accleration.astype('float32')
   dataset_magnitude = Classify_Earthquake_Magnitude.classify_magnitude_for_dataset(dataset_accleration)
   ...
   target = dataset_magnitude.astype('float32')
   ...
   x_train_val, x_test, y_train_val, y_test = train_test_split(data, target, test_size=0.1 ,
                                                       random_state=10, shuffle=True, stratify=target)
   ...
   mode = "gridsearch"
   # 得出最佳的超參數和迭代變化紀錄
   best_parameters, gridsearch_history = RandomForest_Gridsearch.classifier_gridsearch(x_train_val, y_train_val, stratified, for_stratified_magnitude, mode, feature)
   ...
   # 使用最佳的超參數得出交叉驗證結果
   best_param_rf_reg, Accuracy_magnitude_train, Accuracy_magnitude_val = RandomForest_Classifier.classifier_cross_validation(seed, x_train_val, y_train_val, stratified, for_stratified_magnitude, mode, max_depth, n_estimators, feature)
   
   # 使用最佳的超參數套用在原先的訓練集與測試集
   best_param_rf_reg_final, Accuracy_magnitude_final_train, Accuracy_magnitude_test = RandomForest_Classifier.classifier_testing(seed, max_depth, n_estimators, x_test, y_test, x_train_val, y_train_val, feature)
